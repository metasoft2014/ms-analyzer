...
class RNN:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope("embedding", reuse=tf.AUTO_REUSE):
            W = tf.get_variable('W', dtype=tf.float32,
                                initializer=tf.random_uniform((self.vocab_size, self.embedding_size), minval=-1.0,
                                                              maxval=1.0))
...
class LSTM:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope("embedding", reuse=tf.AUTO_REUSE):
            W = tf.get_variable('W', dtype=tf.float32,
                                initializer=tf.random_uniform((self.vocab_size, self.embedding_size), minval=-1.0,
                                                              maxval=1.0), trainable=self.trainable)
...
class CNN:
    ...
    def train(self, input_x, input_y):
        return self.sess.run([self.loss, self.train_op, self.accuracy], feed_dict={self.input_x: input_x, self.input_y: input_y})
